...
class lora_HybridSparseAdapter(nn.Module):
    ...
    def __init__(self, linear, rank, sparsity=0.05, gamma=0.5, sparse_method="svd",
        ...
        super(lora_HybridSparseAdapter, self).__init__()
...
def lora_apply_hybrid_adapter(model, scope="qv", rank=8, sparsity=0.05, gamma=0.5,
        ...
        if hasattr(layer, "self_attn"):
            ...
            for name, sub_module in attn_module.named_children():
                if isinstance(sub_module, nn.Linear) and any(n in name for n in module_names):
                    hybrid_module = lora_HybridSparseAdapter(
...
def lora_get_optimizer_param_groups(model, weight_decay=0.0, lr=0.0002, gamma_lr=0.0001):
    """
